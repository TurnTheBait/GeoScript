...
public class SemanticHandler {
    ...
    public void exportSVG(String filename) {
        ...
        sb.append("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"").append(canvasW).append("\" height=\"")
                .append(canvasH).append("\">\n");
        sb.append("<rect width=\"").append(canvasW).append("\" height=\"").append(canvasH)
                .append("\" fill=\"white\"/>\n");
    ...
    public static interface Expr {
        int eval(SemanticHandler sem);
    }

    public static interface Command {
        void execute(SemanticHandler sem);
    }
    ...
    public static class BinExpr implements Expr {
        ...
        public BinExpr(Expr l, String op, Expr r) {
            left = l;
            this.op = op;
            right = r;
        }
    ...
    public static class VarDeclCommand implements Command {
        ...
        public VarDeclCommand(String id, Expr e) {
            this.id = id;
            expr = e;
        }

        public void execute(SemanticHandler sem) {
            sem.defineVar(id, expr.eval(sem));
        }
    }
    ...
    public static class AssignCommand implements Command {
        ...
        public AssignCommand(String id, Expr e) {
            this.id = id;
            expr = e;
        }

        public void execute(SemanticHandler sem) {
            sem.setVar(id, expr.eval(sem));
        }
    }

    public static class IfCommand implements Command {
        Expr cond;
        List<Command> thenBody;
        List<Command> elseBody;

        public IfCommand(Expr c, List<Command> t, List<Command> e) {
            cond = c;
            thenBody = t;
            elseBody = e;
        }
    ...
    public static class WhileCommand implements Command {
        ...
        public WhileCommand(Expr c, List<Command> b) {
            cond = c;
            body = b;
        }
    ...
    public static class ShapeCommand implements Command {
        ...
        public void execute(SemanticHandler sem) {
            if (!sem.canvasDefined && !kind.equals("TEXT"))
                return;
            if (fill != null && !fill.startsWith("#") && !kind.equals("TEXT"))
                sem.checkColor(fill, fillToken);
            if (stroke != null && !stroke.startsWith("#"))
                sem.checkColor(stroke, strokeToken);

            int[] nums = new int[exprs.length];
            for (int i = 0; i < exprs.length; i++)
                nums[i] = exprs[i].eval(sem);

            // Validation for negative measures
            if (kind.equals("RECT")) {
                if (nums[2] < 0 || nums[3] < 0) {
                    System.err.println(
                            "Error: RECT dimensions cannot be negative (w=" + nums[2] + ", h=" + nums[3] + ")");
                    return;
                }
            } else if (kind.equals("CIRCLE")) {
                if (nums[2] < 0) {
                    System.err.println("Error: CIRCLE radius cannot be negative (r=" + nums[2] + ")");
                    return;
                }
            } else if (kind.equals("SQUARE")) {
                if (nums[2] < 0) {
                    System.err.println("Error: SQUARE size cannot be negative (s=" + nums[2] + ")");
                    return;
                }
            } else if (kind.equals("ELLIPSE")) {
                if (nums[2] < 0 || nums[3] < 0) {
                    System.err.println(
                            "Error: ELLIPSE radii cannot be negative (rx=" + nums[2] + ", ry=" + nums[3] + ")");
                    return;
                }
            }
            ...
            String svg = "";
            if (kind.equals("RECT")) {
                svg = "<rect x=\"" + nums[0] + "\" y=\"" + nums[1] + "\" width=\"" + nums[2] + "\" height=\"" + nums[3]
                        + "\" " + style + " />";
            } else if (kind.equals("CIRCLE")) {
                svg = "<circle cx=\"" + nums[0] + "\" cy=\"" + nums[1] + "\" r=\"" + nums[2] + "\" " + style + " />";
            } else if (kind.equals("LINE")) {
                svg = "<line x1=\"" + nums[0] + "\" y1=\"" + nums[1] + "\" x2=\"" + nums[2] + "\" y2=\"" + nums[3]
                        + "\" " + style + " />";
            } else if (kind.equals("SQUARE")) {
                svg = "<rect x=\"" + nums[0] + "\" y=\"" + nums[1] + "\" width=\"" + nums[2] + "\" height=\"" + nums[2]
                        + "\" " + style + " />";
            } else if (kind.equals("ELLIPSE")) {
                svg = "<ellipse cx=\"" + nums[0] + "\" cy=\"" + nums[1] + "\" rx=\"" + nums[2] + "\" ry=\"" + nums[3]
                        + "\" " + style + " />";
            } else if (kind.equals("TRIANGLE")) {
                svg = "<polygon points=\"" + nums[0] + "," + nums[1] + " " + nums[2] + "," + nums[3] + " " + nums[4]
                        + "," + nums[5] + "\" " + style + " />";
            } else if (kind.equals("POLYGON")) {
                StringBuilder pts = new StringBuilder();
                for (int i = 0; i < nums.length; i += 2)
                    pts.append(nums[i]).append(",").append(nums[i + 1]).append(" ");
                ...
                svg = "<text x=\"" + nums[0] + "\" y=\"" + nums[1] + "\" fill=\"" + textColor + "\">" + fill
                        + "</text>";
            }
